      ...
      <td id="r_sidebar" style="width:300px;" valign="top" >
        <div id="search_div" class="scrollable">
          ...
          <div id="add_song" style="border-left:1px solid black;overflow:auto;">
            <a py:for="song in songs_list" py:attrs="title='t:'+re.sub(r'[^a-zA-Z0-9\s:-]', '', song.title)+'; a:'+(song.author or '!a')+'; c:'+' c:'.join((song.categories or '!c').split(', '))+' '+re.sub(r'[^a-zA-Z0-9\s:-]','',' '.join(song.content.split()))+' '+song.copyright" py:content="song.title" href="${tg.url('song_view#path=%s' % song.path)}" ></a>
          </div>
